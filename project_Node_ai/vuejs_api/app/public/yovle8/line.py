...
def log_pass(name, direction):
    """Log the person passing with their name and direction (in/out) into the database."""
    timestamp = datetime.now()
    try:
        cursor.execute('''
        INSERT INTO passes (name, timestamp, direction)
        VALUES (%s, %s, %s)
        ''', (name, timestamp, direction))
        conn.commit()
    except mysql.connector.Error as err:
        print(f"Database error: {err}")
    print(f"Logged: {name} crossed {direction} at {timestamp}")
        ...
        for idx in range(len(result.boxes.cls)):
            ...
            if trackId not in track:
                ...
                if x_prev["left"] < cx and x_cur_left >= cx:
                    count_in += 1
                    log_pass(name, "in")  # Use recognized name
                elif x_prev["right"] > cx and x_cur_right <= cx:
                    count_out += 1
                    log_pass(name, "out")  # Use recognized name
