...
def log_pass(name, direction, frame):
    ...
    filename = f"vuejs_api/app/public/yovle8/captures/{name}_{direction}_{timestamp.strftime('%Y%m%d_%H%M%S')}.jpg"
    ...
    filename_db = f"captures/{name}_{direction}_{timestamp.strftime('%Y%m%d_%H%M%S')}.jpg"
    
    try:
        cursor.execute('''
        INSERT INTO passes (time_id, name, timestamp, direction, image_path)
        VALUES (%s, %s, %s, %s, %s)
        ''', (time_id, name, timestamp, direction, filename_db))
        conn.commit()
    except mysql.connector.Error as err:
